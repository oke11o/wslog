...
func Example() {
	slog.SetDefault(slog.New(wslog.NewHandler(os.Stderr, &wslog.Options{
	...
	slog.Error("DB connection lost", wslog.Err(errors.New("connection reset")), "db", "myapp")
	// Output:
}
...
func TestHandler(t *testing.T) {
	...
	tests := []struct {
		...
		{
			F: func(l *slog.Logger) {
				l.Error("test", wslog.Err(errors.New("fail")))
			},
			Want: `Nov 10 23:00:00.000 ERR test err=fail`,
		},
		{
			F: func(l *slog.Logger) {
				l.Info("test", slog.Group("group", slog.String("key", "val"), wslog.Err(errors.New("fail"))))
			},
		...
		},
		{
			...
			Want: `Nov 10 23:00:00.000 INF wslog/handler_test.go:100 test key=val`,
		},
		{
			Opts: &wslog.Options{
				TimeFormat: time.Kitchen,
		...
		{ // https://github.com/lmittmann/tint/issues/37
			...
			Want: `Nov 10 23:00:00.000 INF wslog/handler_test.go:327 test`,
		},
		{ // https://github.com/lmittmann/tint/issues/44
			F: func(l *slog.Logger) {
				l = l.WithGroup("group")
				l.Error("test", wslog.Err(errTest))
			},
		...
		{ // https://github.com/lmittmann/tint/pull/66
			F: func(l *slog.Logger) {
				errAttr := wslog.Err(errors.New("fail"))
				errAttr.Key = "error"
	...
	for i, test := range tests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			if test.Opts == nil {
				test.Opts = &wslog.Options{}
			}
			test.Opts.NoColor = true
			l := slog.New(wslog.NewHandler(&buf, test.Opts))
			test.F(l)
...
func TestReplaceAttr(t *testing.T) {
	...
	for i, test := range tests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			tintLogger := slog.New(wslog.NewHandler(io.Discard, &wslog.Options{
...
func BenchmarkLogAttrs(b *testing.B) {
	handler := []struct {
		...
		{"wslog", wslog.NewHandler(io.Discard, nil)},
